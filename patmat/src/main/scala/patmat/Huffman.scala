...
object Huffman {
      ...
      def place(list: List[CodeTree]): List[CodeTree] = {
        if (list.isEmpty)
          ...
          if (fork.weight <= weight)
            ...
            head :: place(list.tail)
        }
      }

      place(trees.tail.tail)
    }
  }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    ...
    def encodeChar(node: CodeTree, char: Char, bits: List[Bit]): List[Bit] = {

      def containsChar(node: CodeTree): Boolean = {
        node match {
    ...
    def encodeRec(chars: List[Char], bits: List[Bit]): List[Bit] = {
      if (chars.isEmpty) bits
      else encodeChar(tree, chars.head, Nil) ::: encodeRec(chars.tail, bits)
    }
  ...
  def convert(tree: CodeTree): CodeTable = {
    def convertRec(node: CodeTree, bits: List[Bit]): CodeTable = {
      node match {
        case Leaf(char, _) => List((char, bits.reverse))
        case Fork(left, right, char, _) => convertRec(left, 0 :: bits) ::: convertRec(right, 1 :: bits)
      }
    }
  ...
  def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    ...
    text.foldLeft(List[Bit]())((bits: List[Huffman.Bit], char: Char) => codeBits(codeTable)(char) ::: bits).reverse
  }
}
